WITH per_count AS (
	...
	t.ner_xml AS title, e.directory, e.published, w.liste_type, TRANSLATE(lower((COALESCE(s.loc,'') || ' '::text) || COALESCE(s.cardinal,'')),'éëèêàçôö,.:;\-_\(\)\[\]?!$&','eeeeacoo') AS fulladd, s.index AS id_address, g."precise.geo_response" AS precise_geo_response, g."precise.geom" AS precise_geom
